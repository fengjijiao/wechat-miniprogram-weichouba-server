...
function getSearchSuggestions(req,res,next,sqltool) {
	var result={};
	var location="0,6";
    ...
    if(req.query.key){
    	sqltool.query("SELECT * FROM `publicwelfares` WHERE `wName` LIKE '"+req.query.key+"%' OR `wName` LIKE '%"+req.query.key+"%' OR `wName` LIKE '%"+req.query.key+"' ORDER BY `timestamp` DESC LIMIT "+location+";",(publicwelfares) => {
    		result.inpuTips=[];
            result.inpuTips_length=publicwelfares.length;
    		if(publicwelfares.length<1){
                res.end(JSON.stringify({status:1,msg:"No More PublicWelfare!",data:result}));
            }
...
function getPublicWelfares(req,res,next,sqltool) {
	var result={};
	var location="0,10";
	if(req.query.prev && !isNaN(req.query.prev) && req.query.last && !isNaN(req.query.last)){
       location=req.query.prev+","+req.query.last;
